...
* Of note both Professor Trash wheel and Gwynnda have missing values (NA) for sports balls as they did not have that column to begin with, and glass bottles for Gqynnda as well has NA.
...
* To keep only those that met selection criteria: first we selected those that did develop MCI by removing those with missing values of NA in the `age_of_onset` variable, and made a new table `Baseline_demographics_with_MCI.df`. 
  ...
  * Next we kept only those with `current_age` < `age_at_onset`, which removed three samples that had developed MCI before current age at enrollment. 
...
Baseline_demographics_with_MCI.df <- subset(Baseline_demographics_with_MCI.df, current_age < age_at_onset)
...
There are `r nrow(Baseline_demographics_with_MCI.df %>% filter(shared_with_biomarkers == "FALSE"))` IDs from the `Baseline_demographics_without_MCI.df` not found in the `Amyloid_biomaker` (id = 14, 49, 268). 

There are `r nrow(Amyloid_Biomarker.df %>% filter(shared_with_baseline == "FALSE"))` IDs from the `Amyloid_biomarker` not found in the `Baseline_demographics_with_MCI.df`.

## Combining the Datasets

We will use the `inner_join` function to combine the two tables of `Baseline_demographics_with_MCI.df` and `Amyloid_Biomarker.df`, so that only those with participants found in both are retained. 
