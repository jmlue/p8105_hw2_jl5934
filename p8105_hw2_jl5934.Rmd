...
* The number of participants recruited were `r nrow(Baseline_demographics.df)` patients, but of those that had MCI it was `r nrow(Baseline_demographics_with_MCI.df)`.

* Baseline mean age of all participants was `r mean(Baseline_demographics.df[["current_age"]])`, and for those with MCI it was `r mean(Baseline_demographics_with_MCI.df[["current_age"]])`. Range of the ages for all participants was `r range(Baseline_demographics.df[["current_age"]])` and range of ages for those with MCI was `r mean(Baseline_demographics_with_MCI.df[["current_age"]])`.

* The proportion of women in the study who are APOE4 carriers are `r (nrow(Baseline_demographics.df %>% filter(sex == '0' & apoe4 == '1')))/(nrow(Baseline_demographics.df %>% filter(sex == '0')))` from the total population, and `r (nrow(Baseline_demographics_with_MCI.df %>% filter(sex == '0' & apoe4 == '1')))/(nrow(Baseline_demographics_with_MCI.df %>% filter(sex == '0')))` amongst those with MCI.
...
We will determine which sample IDs are found in Baseline Demographics but not in amyloid, and then coversely see which are in Amyloid Biomarkers but not in Baseline Demographics. 
...
There are `r nrow(Baseline_demographics.df %>% filter(shared_with_biomarkers == "FALSE"))` IDs from the baseline demographics not found in the amyloid biomakers (id = 14, 49, 92, 179, 268, 304, 389, 412). 

There are `r nrow(Amyloid_Biomarker.df %>% filter(shared_with_baseline == "FALSE"))` IDs from the amyloid biomarker dataframe not found in the baseline demographics (id = 484-495).
...
When looking at this new combined dataset it takes data based on Sample ID and now includes both demographic information, as well as clinical biomarkers, only when sample ID data is available for both. 

* The variables included are `r names(Baseline_Amyloid.df)`.

* The size of the dataframe is `r nrow(Baseline_Amyloid.df)` rows by `r ncol(Baseline_Amyloid.df)` columns. 
