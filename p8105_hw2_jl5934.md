...
- Of note both Professor Trash wheel and Gwynnda have missing values
  ...
  and glass bottles for Gqynnda as well has NA.
...
- To keep only those that met selection criteria: first we selected
  ...
  - Next we kept only those with `current_age` \< `age_at_onset`, which
    removed three samples that had developed MCI before current age at
    enrollment.
...
There are 3 IDs from the `Baseline_demographics_without_MCI.df` not
found in the `Amyloid_biomaker` (id = 14, 49, 268).
